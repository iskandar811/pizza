...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['checkout'])) {      
        ...
        $sql2 = "INSERT INTO aes_key (phone, enc_key) VALUES (?,?)";
        if($stmt2 = mysqli_prepare($conn2, $sql2)){
            mysqli_stmt_bind_param($stmt2, "ss", $phone,$enc_key);
        ...
        $ephone=openssl_encrypt($phone, 'AES-256-CBC', $enc_key, OPENSSL_RAW_DATA);
        //AES end
        ...
        $sql = "INSERT INTO `orders` (`userId`, `address`, `zipCode`, `phoneNo`, `amount`, `paymentMode`, `orderStatus`, `orderDate`) VALUES ('$userId', '$address', '$zipcode', '$ephone', '$amount', '0', '0', current_timestamp())";
        $result = mysqli_query($conn, $sql);
        ...
        if ($result){
            ...
            header("Location:/spasstwo/stripe/create-checkout-session.php");
            exit();
            echo '<script>alert("Thanks for ordering with us. Your order id is ' .$orderId. '.");
                window.location.href="http://localhost/spasstwo/index.php";  
                </script>';
                exit();
        }
        else{
